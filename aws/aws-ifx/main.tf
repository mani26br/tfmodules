...
module "ssm_InstallCloudWatchAgent" {
  ...
  key = "InstanceIds"
  values = ["*"]
  schedule_expression = "at(2023-08-24T22:00:00)"
...
module "sns_cloudwatch_alarms_notifications"{
  source = "../../terraform-modules/aws/sns/sns_topics"
  sns_topic_name = "aws-ifx-cloudwatch-security-alarms"
  sns_topic_display_name = "aws-ifx-cloudwatch-security-alarms"
  sns_topic_tags = var.common_tags
  sns_topic_policy = data.aws_iam_policy_document.cw_sns_topic_policy.json 
}

module "cloudwatch_alarms_sns_topic_email_subscription"{
  source = "../../terraform-modules/aws/sns/sns_topic_subscription"
  sns_topic_subscription_topic_arn = module.sns_cloudwatch_alarms_notifications.sns_topic_arn
  sns_topic_subscription_protocol = "email"
  sns_topic_subscription_endpoint =  var.guardduty_subscription_email
}
...
module "cloudwatch_alarms" {
  ...
  depends_on = [ module.sns_cloudwatch_alarms_notifications ]
  for_each = local.CloudTrailMetrics
  alarm_name = "${each.key}"
  metric_name = "${each.key}"
  namespace = var.metric_namespace
  alarm_actions = [module.sns_cloudwatch_alarms_notifications.sns_topic_arn]
  tags = var.common_tags
}
